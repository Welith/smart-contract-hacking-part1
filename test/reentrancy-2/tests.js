...
describe("Reentrancy Exercise 2", function () {
  ...
  it("Exploit", async function () {
    ...
    const ApesAttackFactory = await ethers.getContractFactory(
      "ApesAttack",
      attacker
    );

    this.attack = await ApesAttackFactory.deploy(
    ...
    console.log(
      "Attacker balance",
      (await this.airdrop.balanceOf(this.attack.address)).toString()
    );
  });
