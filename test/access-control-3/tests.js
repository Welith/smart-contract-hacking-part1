...
describe("Access Control Exercise 3", function () {
  ...
  it("Users are purchasing fragrences", async function () {
    ...
    await this.kilian
      .connect(user1)
      .purchaseFragrance(1, { value: FRAGRENCE_PRICE });
    await this.kilian
      .connect(user1)
      .purchaseFragrance(4, { value: FRAGRENCE_PRICE });

    await this.kilian
      .connect(user2)
      .purchaseFragrance(2, { value: FRAGRENCE_PRICE });
    await this.kilian
      .connect(user2)
      .purchaseFragrance(3, { value: FRAGRENCE_PRICE });

    await this.kilian
      .connect(user3)
      .purchaseFragrance(5, { value: FRAGRENCE_PRICE });
    await this.kilian
      .connect(user3)
      .purchaseFragrance(8, { value: FRAGRENCE_PRICE });
  });
  ...
  after(async function () {
    ...
    expect(attackerBalance).to.be.gt(
      this.attackerInitialETHBalance
        .add(ethers.utils.parseEther("10").mul(6))
