...
describe("Reentrancy Exercise 1", function () {
  ...
  it("Exploit", async function () {
    ...
    this.bankAttack = await AttackerFactory.deploy(this.bank.address);

    await this.bankAttack.attack({ value: ethers.utils.parseEther("1") });

    console.log("test", await this.bank.balances(this.bankAttack.address));

    await this.bankAttack.withdraw();
  });
